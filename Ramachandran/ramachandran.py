...
for index in range(0,12):
    ...
    import Bio.PDB
    structure = Bio.PDB.PDBParser().get_structure(pdb_code, "frames/out_{}.pdb".format(str(index)))
    print("Done")

    print("About to save angles to file...")
    output_file = open("tsv/out_{}.tsv".format(index),"w")
    for model in structure :
        for chain in model :
            print("Chain {}".format(str(chain.id)))
